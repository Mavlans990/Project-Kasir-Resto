...
$kode = $_GET['kode'];
$psnl = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT a.*, d.* FROM detail_transaksi AS a
  JOIN menu    AS d ON a.id_menu = d.id_menu WHERE kode_transaksi = '$kode' "));

$mj = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM transaksi WHERE kode_transaksi = '$kode'"));
                    ...
                    <div class="row">
                      <div class="mb-3 col-md-6">
                        ...
                        <div class="input-group">
                          ...
                          <input type="text" class="form-control" name="nomor_meja" value="<?= $mj['nomor_meja'] ?>" > 
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div class="card-body">
            <div class="table-responsive">
              ...
              <tbody>
                <?php
                $no = 1;
                $sql = mysqli_query($koneksi, "SELECT a.*, b.* FROM detail_transaksi a
                ...
                while ($row = mysqli_fetch_assoc($sql)) {
                  ...
                  <tr>
                    ...
                    <td>
                      <a href="<?= 'hapus_transaksi_lanjut.php?id=' . $row['id_detail']?>" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
            ...
            <div class="col-lg-4 col-sm-5 ms-auto">
              <table class="table table-clear">
                <tbody>
                  <tr>
                    <?php
                    $meja = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM transaksi WHERE kode_transaksi = '$kode'"))
                    ?>
                    <td class="left"><strong>Nomer Meja</strong></td>
                    <td class="right"><?= $meja['nomor_meja']?></td>
                  </tr>
                  <tr>
                    <?php
                  ...
                  <tr>
                    ...
                    <td class="right"> <input type="text" name="bayar" onkeyup="OnChange(this.value)" onKeyPress="return isNumberKey(event)">  </td>
                  </tr>
                  <tr>
                    <td class="left"><strong>Kembali</strong></td>
                    <td class="right"> <input type="text" name="kembali" readonly>  </td>
                  </tr>
                </tbody>
              </table>
            </div>
...
<!--**********************************
  ...
  <script type="text/javascript" language="Javascript">
   totalbayar = document.formD.jumlah_total.value;
   ...
   function OnChange(value){
     ...
     document.formD.kembali.value = total;
   }
 </script>
...
if (isset($_POST['simpan'])) {
  ...
  $sql = mysqli_query($koneksi, " INSERT INTO detail_transaksi VALUES('', '$kode_transaksi', '$nomor_meja', '$id_menu', '$jumlah_pesanan', '$total_hrg')");
...
if (isset($_POST['edit'])) {
  ...
  if ($bayar < $jumlah_total){
    echo "
    <script>
    alert ('Maaf , Pembayaran Anda Kurang');
    </script>
    ";
  }else{
    echo"
